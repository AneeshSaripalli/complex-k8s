...
kubectl apply -f k8s
kubectl set image deployments/client-deployment server=aneeshsaripalli/multi-client:$SHA
kubectl set image deployments/server-deployment server=aneeshsaripalli/multi-server:$SHA
kubectl set image deployments/worker-deployment server=aneeshsaripalli/multi-worker:$SHA
